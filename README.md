...
If we test the builder and input a name of "Tool", a description of "Hammer", a price of "10.5" and an _id of "99" we can expect a object to be created that matches this format.

#### Integration 

An example Integration test is included in the project.

For integration tests we can tes the RESTful endpoints.
...
If we test the **CREATE** endpoint by sending a request with a method of `POST` and a path of `/create` we should expect the response to be...Status code: 201, Status text: Created


#### Use Acceptance Tetsing

This is similar to a 'black-box' system test as we are testing the front-end with no knowledge of the back-end.

An example of a user acceptance test is included with this project.
